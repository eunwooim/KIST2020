...
prev_dirs = ['APTech','CHN','Jang Sky','JHSeo','Jo Min Kyung','JSKim','JW Cho','JY Hong','Chad','HSKim','JD Kim','JK Choi','JPAhn','Juyoung Jeong','JW Lim']
...
for folder, filename in walk(walkdir):
  ...
  v = abs(x).max()/10
  gray = x.astype(np.double)
  gray[x>v] = v
  gray /= gray.max()
  gray *= 255
  gray = gray.astype(np.uint8)
  gray = cv2.resize(gray,dsize=(800, 800), interpolation=cv2.INTER_AREA)
  ret, dest = cv2.threshold(gray, 100, 55, cv2.THRESH_BINARY) #(img,임계값,밝기,BINARY)
  circles = cv2.HoughCircles(dest, cv2.HOUGH_GRADIENT, 1, 500, param1 = 310, param2 = 11, minRadius = 40, maxRadius = 150)
  if circles is None:
    print("%s" % filepath)
  else:
    print("# %s" % filepath)
